...
int get_index(struct machine* machine, struct chunk* chunk) {
	if (machine->evals < 2) {
		machine->last_err = error_insufficient_evals;
		return 0;
	}
...
int set_index(struct machine* machine, struct chunk* chunk) {
	if (machine->evals < 3) {
		machine->last_err = error_insufficient_evals;
		return 0;
	}
...
int execute(struct machine* machine, struct chunk* chunk) {
	while (!end_chunk(chunk))
	{
		...
		case MACHINE_GET_INDEX:
			if(!get_index(machine, chunk))
				return machine->last_err;
			break;
		case MACHINE_SET_INDEX:
			if (!set_index(machine, chunk))
				return machine->last_err;
			break;
